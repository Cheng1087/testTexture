...
vtkStandardNewMacro(vtkECPolyDataNormals);

// Construct with feature angle=30, splitting and consistency turned on,
// flipNormals turned off, and non-manifold traversal turned on.
vtkECPolyDataNormals::vtkECPolyDataNormals()
{
...
int vtkECPolyDataNormals::RequestData(
...
//    if(!this->AutoGenerateCenter)
//    {
//        vtkDebugMacro(<<"not set Center yet!");
//        return 1;
//    }
//    generate center
    input->GetCenter(this->Center);
	//this->ComputeCenter(output);
	...
	//	move setpoints there so when compute point normal can get the specific point.
	...
	double curPt[3], dir[3],bounds[6], boundLenth[3];
	output->GetBounds(bounds);
    vtkDebugMacro( << bounds[0] << "_"
		<< bounds[1] << "_"
		<< bounds[2] << "_"
		<< bounds[3] << "_"
		<< bounds[4] << "_"
	...
	boundLenth[0] = bounds[1] - bounds[0];
	boundLenth[1] = bounds[3] - bounds[2];
	boundLenth[2] = bounds[5] - bounds[4];
    vtkDebugMacro( << boundLenth[0] << "_"
		<< boundLenth[1] << "_"
        << boundLenth[2] << "_" << endl);
	if (this->ComputePointNormals)
	{
			...
			++cellId)
		{
			...
			{
				output->GetPoint(pts[i], curPt);
				vtkMath::Subtract(curPt, this->Center, dir);
				dir[0] /= boundLenth[0];
				dir[1] /= boundLenth[1];
				dir[2] /= boundLenth[2];
				
				fNormals[3 * pts[i]] = dir[0];
				fNormals[3 * pts[i] + 1] = dir[1];
				fNormals[3 * pts[i] + 2] = dir[2];
		...
		//{
		//	fNormals[3 * i] /= boundLenth[0];
		//	fNormals[3 * i + 1] /= boundLenth[1];
		//	fNormals[3 * i + 2] /= boundLenth[2];
		//	
...
//
void vtkECPolyDataNormals::TraverseAndOrder(void)
{
	vtkIdType i, k;
	int j, l, j1;
...
//
void vtkECPolyDataNormals::MarkAndSplit(vtkIdType ptId)
{
	int i, j;
...
void vtkECPolyDataNormals::ComputeCenter(vtkDataSet *output)
{
	double p[3];
	...
	cout << "Center computed as: (" << this->Center[0] << ", "
		<< this->Center[1] << ", "
		<< this->Center[2] << ")" << endl;
...
void vtkECPolyDataNormals::SetCenter(double center[3])
{
    this->Center[0] = center[0];
    this->Center[1] = center[1];
    this->Center[2] = center[2];
    this->AutoGenerateCenter = 0;
}

void vtkECPolyDataNormals::PrintSelf(ostream& os, vtkIndent indent)
{
